...
export async function verifyImageMatchesGoal({
  ...
  mimeType,
  goal,
  // 15s per attempt × up to 2 attempts is well inside the route's
  // maxDuration=30s budget while leaving room for EXIF parse, Mongo
  // write, and the XRPL EscrowFinish call.
  timeoutMs = 15_000,
  maxAttempts = 2,
}) {
